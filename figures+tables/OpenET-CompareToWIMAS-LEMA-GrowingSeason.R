## OpenET-CompareToWIMAS-LEMA-GrowingSeason.R
# This script is supposed to get total irrigation within SD-6 LEMA 
# and compare to WIMAS output.Uses irrigation estimates based on 
# growing season only ET and met data.

source(file.path("code", "paths+packages.R"))

## load data
# field attributes
fields_spatial <- 
  readr::read_csv(file.path("data", "Fields_Attributes-Spatial.csv"))

# et data
df_et_gs <- readr::read_csv(file.path(dir_data, "OpenET", "Monthly_2016-2021", "OpenET_EstimateFieldIrrigation-GrowingSeason_FieldsNoDups.csv")) # 2016-2021

# wimas data
wimas_wuse <- readr::read_csv(file.path(dir_data, "WIMAS_WaterUse_1990to2020_SD6+10mi_Summarize.csv"))

# join together
lema_fields_data <-
  df_et_gs |> 
  left_join(fields_spatial, by = "UID") |> 
  subset(within_lema)

# set irrigation threshold and subset to irrigated fields only
irr_thres <- 0.5 # 0-1
lema_irr_fields <- 
  subset(lema_fields_data, IrrigatedPrc > irr_thres)

# sum by year and algorithm
lema_irr_totals <-
  lema_irr_fields |> 
  group_by(Year, Algorithm) |> 
  summarize(irr_m3_fromNonIrr_total = sum(irr_m3_fromNonIrr),
            irr_m3_fromPrec_total = sum(irr_m3_fromPrec),
            n_fields = n())

# combine with wimas
irr_openet_wimas <-
  left_join(lema_irr_totals, wimas_wuse, by = "Year") |> 
  rename(Irr_m3_WIMAS = TotalVolume_m3)

# save
write_csv(irr_openet_wimas, file.path("figures+tables", "OpenET-CompareToWIMAS-LEMA-GrowingSeason_TotalIrrigation.csv"))

# plot
p_scatter_fromPrec <- 
  ggplot(irr_openet_wimas, aes(y = irr_m3_fromPrec_total/1e6, x = Irr_m3_WIMAS/1e6)) +
  geom_abline(intercept = 0, slope = 1, color = col.gray) +
  geom_point(aes(color = Algorithm)) +
  scale_y_continuous(name = "Estimated Irrigation (growing season ET - Prec) [million m\u00b3]") +
  scale_x_continuous(name = "WIMAS Reported Irrigation [million m\u00b3]") +
  scale_color_brewer(name = "Algorithm", type = "qual", labels = labs_algorithms) +
  stat_smooth(method = "lm") +
  theme(legend.position = "bottom")

p_line_fromPrec <-
  ggplot(irr_openet_wimas) +
  # WIMAS data
  geom_point(aes(x = Year, y = Irr_m3_WIMAS/1e6), color = "black", size = 2) +
  geom_line(aes(x = Year, y = Irr_m3_WIMAS/1e6), color = "black", size = 2) +
  # OpenET data
  geom_point(aes(x = Year, y = irr_m3_fromPrec_total/1e6, color = Algorithm)) + 
  geom_line(aes(x = Year, y = irr_m3_fromPrec_total/1e6, color = Algorithm), show.legend = F) +
  # aesthetics
  scale_y_continuous(name = "Estimated Irrigation (growing season ET - Prec) [million m\u00b3]") +
  scale_color_brewer(name = "Algorithm", type = "qual", labels = labs_algorithms) +
  theme(legend.position = "bottom")

p_combo_fromPrec <-
  p_scatter_fromPrec + p_line_fromPrec +
  plot_layout(ncol = 2, guides = "collect") &
  theme(legend.position = "bottom")
p_combo_fromPrec

ggsave(file.path("figures+tables", "OpenET-CompareToWIMAS-LEMA-GrowingSeason_IrrFromPrecip.png"),
       p_combo_fromPrec, width = 17.15, height = 11.15, units = "cm")



p_scatter_fromNonIrr <- 
  ggplot(irr_openet_wimas, aes(y = irr_m3_fromNonIrr_total/1e6, x = Irr_m3_WIMAS/1e6)) +
  geom_abline(intercept = 0, slope = 1, color = col.gray) +
  geom_point(aes(color = Algorithm)) +
  scale_y_continuous(name = "Estimated Irrigation (growing season ET - NonIrr ET) [million m\u00b3]") +
  scale_x_continuous(name = "WIMAS Reported Irrigation [million m\u00b3]") +
  scale_color_brewer(name = "Algorithm", type = "qual", labels = labs_algorithms) +
  stat_smooth(method = "lm") +
  theme(legend.position = "bottom")

p_line_fromNonIrr <-
  ggplot(irr_openet_wimas) +
  # WIMAS data
  geom_point(aes(x = Year, y = Irr_m3_WIMAS/1e6), color = "black", size = 2) +
  geom_line(aes(x = Year, y = Irr_m3_WIMAS/1e6), color = "black", size = 2) +
  # OpenET data
  geom_point(aes(x = Year, y = irr_m3_fromNonIrr_total/1e6, color = Algorithm)) + 
  geom_line(aes(x = Year, y = irr_m3_fromNonIrr_total/1e6, color = Algorithm), show.legend = F) +
  # aesthetics
  scale_y_continuous(name = "Estimated Irrigation (growing season ET - NonIrr ET) [million m\u00b3]") +
  scale_color_brewer(name = "Algorithm", type = "qual", labels = labs_algorithms) +
  theme(legend.position = "bottom")

p_combo_fromNonIrr <-
  p_scatter_fromNonIrr + p_line_fromNonIrr +
  plot_layout(ncol = 2, guides = "collect") &
  theme(legend.position = "bottom")
p_combo_fromNonIrr

ggsave(file.path("figures+tables", "OpenET-CompareToWIMAS-LEMA-GrowingSeason_IrrFromNonIrr.png"),
       p_combo_fromNonIrr, width = 17.15, height = 11.15, units = "cm")
